 ...
 */
app.get('/api/nearbyCrimes', (req, res) => {
    ...
    req.coords.forEach(coord => {
        ...
        const sqlQuery = `SELECT * FROM \`hackwestern2018.chicago_crime.crime\` WHERE latitude>=${latMin} AND latitude<=${latMax} AND longitude>=${lonMin} AND longitude<=${lonMax} AND date BETWEEN \"2017-11-25\" AND \"2018-11-25\"`;
        const options = {
            query: sqlQuery,
            useLegacySql: false,
        };
